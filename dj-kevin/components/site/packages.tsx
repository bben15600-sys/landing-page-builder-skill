      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <Reveal className="text-center mb-14">
          ...
          <h2 className="font-display font-extrabold text-4xl sm:text-5xl md:text-6xl leading-tight">
            בוחרים <span className="neon-gradient">פורמט</span>
          </h2>
          <p className="mt-6 text-muted-foreground max-w-xl mx-auto">
            המחירים הם נקודת התחלה. כל הצעה נבנית לפי גודל האירוע, האולם ומה
            שחשוב לכם.
          </p>
        </Reveal>
          ...
          {siteConfig.packages.map((pkg, idx) => (
            <Reveal key={pkg.name} delay={idx * 80} className="h-full">
              <div
                className={cn(
                  ...
                  "featured" in pkg && pkg.featured
                    ? "border-accent bg-card ring-glow"
              ...
              >
                ...
                <div className="font-display font-bold text-2xl">{pkg.name}</div>
                <div className="mt-1 text-sm text-muted-foreground">
                  {pkg.hours}
                </div>
                <div className="mt-5 font-display font-extrabold text-4xl">
                  {pkg.price}
                </div>
