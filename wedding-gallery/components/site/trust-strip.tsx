      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 py-12 sm:py-16">
        <Reveal>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 sm:gap-12">
            <Stat
              icon={<Camera size={22} />}
              value={siteConfig.stats.weddings}
              suffix="+"
              label="חתונות"
            />
            <Stat
...
function Stat({
  ...
  return (
    <div className="flex flex-col items-center text-center gap-2">
      ...
      <div className="font-display text-4xl sm:text-5xl text-accent tabular-nums leading-none">
        <AnimatedCounter value={value} suffix={suffix} format={!decimal} />
      </div>
...
function StaticStat({
  ...
  return (
    <div className="flex flex-col items-center text-center gap-2">
      <div className="text-accent">{icon}</div>
      <div className="font-display text-2xl sm:text-3xl text-accent leading-tight">
        {value}
      </div>
      <div className="text-xs sm:text-sm text-muted-foreground tracking-wider">
        {label}
